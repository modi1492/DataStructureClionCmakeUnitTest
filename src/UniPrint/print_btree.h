...
//元素类型
static void
printBTree(BTNodePosi(T) bt, int depth, bool isLeftmost, bool isRightmost, Bitmap *leftmosts, Bitmap *rightmosts) {
    if (!bt) return;
    ...
    while (0 < k--) { //自右向左，输出各子树及其右侧的关键码
        ...
        for (int i = 0; i < depth; i++) //根据相邻各层
            (leftmosts->test(i) && leftmosts->test(i + 1) || rightmosts->test(i) && rightmosts->test(i + 1))
            ? //的拐向是否一致，即可确定
